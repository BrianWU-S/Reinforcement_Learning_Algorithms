...
class PPO(object):
    def __init__(self, A_BOUND, s_dim=3, a_dim=1, hidden_units=100, LR_A=0.0001, LR_C=0.0002, action_update_steps=10,
                 critic_update_steps=10, beta_low=1. / 1.5, beta_high=1.5, alpha=2.,
                 method_index=0, output_graph=True, epsilon=0.2, kl_target=0.01, lam=0.5
                 ):
        ...
        self.alpha = alpha
        self.method = [
            dict(name='kl_penalty', kl_target=kl_target, lam=lam),
            dict(name='clipping', epsilon=epsilon)
        ][method_index]
    ...
    def build_critic_net(self):
        l1 = tf.layers.dense(inputs=self.s, units=self.hidden_units, activation=tf.nn.relu,
    ...
    def update(self, s, a, r):
        ...
        if self.method['name'] == 'kl_penalty':
            ...
            # If kl_mean is large, it means that policy update is large, we need to increase the KL penalty item
