...
class ACNet(object):
    def __init__(self, scope, globalAC=None, ENTROPY_BETA=0.01):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('critic'):  # only critic controls the rnn update
            ...
            cell_out = tf.reshape(outputs, [-1, 64], name='flatten_rnn_outputs')  # joined state representation
    ...
    def choose_action(self, s, cell_state):  # run by a local
        return SESS.run([self.A, self.final_state], {self.s: s[np.newaxis, :], self.init_state: cell_state})


class Worker(object):
    def __init__(self, name, globalAC, MAX_EP_STEP=200, GAMMA=0.9):
