...
def gym_run(env, ddpg, max_action, min_action):
    render = False
    var = max_action
    ...
    for episode in range(EPISODE):
        ...
        for j in range(step_per_epoch):
            ...
            ddpg.store_transition(observation, action, reward / 10, observation_)  # reward factor
            if ddpg.memory_counter > max_exp_step:
                var *= 0.999995  # decay the action randomness
                ddpg.learn()
            reward_sum += reward
...
def run_DDPG(env_name='Pendulum-v0'):
    ...
    gym_run(env, ddpg, max_action=max_action, min_action=min_action)
...
if __name__ == '__main__':
    ...
    env_name = "Ant-v1"
    seed = 0
    memory_size = 100000  # more complex problem: 1000000
    hidden_size = 256
    LR_A = 0.0001
    ...
    TAU = 0.005  # soft replacement, default: 0.01
    EPISODE = 100
    start_step = 25000
    step_per_epoch = 2500
    batch_size = 256  # depend on your running environment, set to a smaller value if GPU memory is not big enough
    render_threshold = 4000
    # training
    run_DDPG(env_name=env_name)
